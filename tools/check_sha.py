...
# files (e.g. nongoogle.bzl) by checking for physical assets in
...
#  -c/--check           Will just validate the contents of nongoogle.bzl
#                       and report on missing or duplicated SHAs.
#  --ignore_errors      By default we are strict on checking and exit with an error code to fail
#                       the build for issues such as out-of-date dependencies at release. If this flag
#                       is set we won't exit with an error to allow local/snapshot builds to proceed with a warning.
...
#                       file.
#  -p/--patch           This flag will update assets in nongoogle.bzl
...
# artifact.
default_filter = r'.*(com\.wandisco|org\.eclipse\.jgit).*'
...
def assets_from_bazel_file(filename):
    ...
    with open(filename) as file_in:
        ...
        for line in file_in:
            ...
            if re.match(r"^maven_jar\(.*$", line):
                current_asset = Asset()
            if current_asset and re.match(r"^\)$", line):
...
def update_bazel_file(filename, assets_to_update):
    ...
    for count, original_line in enumerate(data):
        ...
        if re.match(r"^maven_jar\(.*$", line):
            ...
            current_asset = Asset()
        elif current_asset and re.match(r"^\)$", line):
            ...
            if not current_asset:
                ...
                if k == "artifact" and re.match(r".*:.*\+", v):
                    # Get variable name
...
def parse_jgit_version(version):
    ...
    if not version:
        ...
        return None
    elif re.search(r"^\d+\.\d+\.\d+-.*$", version) or re.search(r"^\d+\.\d+\.\d+\..*$", version):
        return re.split('[.-]', version.strip(), 3)
    else:
        print("JGit version does not begin with 'x.x.x', version=" + version, file=stderr)
        return None

def read_jgit_docs_version(filename):
    """
    Gets the doc version from a file
    :param filename: The name of the file to read
    :return: The docs version.
    """
    with open(filename) as file:
        for line in file:
            if line.startswith("_DOC_VERS = "):
                return re.sub('.* = |"', '', line.strip())

    print("Could not find the _DOC_VERS property in " + filename, file=stderr)
    return None

def check_jgit_docs_version(assets_to_update, skipped_assets):
    """
    Checks the JGit docs version against the JGit version from modules/jgit/pom.xml
    :param assets_to_update: The set of assets to be updated.
    :param skipped_assets: The set of skipped assets.
    :return: Updated sets of updated and skipped assets.
    """

    # Get the submodule version
    submodule_version = eval_maven_project_version("modules/jgit/pom.xml")
    jgit_submodule_version = parse_jgit_version(submodule_version)
    if not jgit_submodule_version:
        print("Couldn't parse the JGit version from output: version=" + submodule_version, file=stderr)
        return assets_to_update, skipped_assets

    # Get the docs version
    docs_version = read_jgit_docs_version("java/com/google/gerrit/extensions/BUILD")
    jgit_docs_version = parse_jgit_version(docs_version)
    if not jgit_docs_version:
        print("Couldn't parse the JGit docs version. version=" + docs_version, file=stderr)
        return assets_to_update, skipped_assets

    # Create JGit asset
    jgit_asset = Asset()
    jgit_asset.name = "jgit"
    jgit_asset.artifact = "org.eclipse.jgit:jgit:" + '.'.join([jgit_submodule_version[0], jgit_submodule_version[1]])

    # Compare each version, if any differences handle appropriately.
    versions_match = False
    if jgit_submodule_version[0] != jgit_docs_version[0]:
        print("JGit submodule and docs major version does not match, bom=" + submodule_version + ", docs=" + docs_version, file=stderr)
    else:
        if jgit_submodule_version[1] != jgit_docs_version[1]:
            print("JGit submodule and docs minor version does not match, bom=" + submodule_version + ", docs=" + docs_version, file=stderr)
        versions_match = True

    if versions_match:
        skipped_assets.update({jgit_asset.name: jgit_asset})
    else:
        jgit_asset.old_version = '.'.join([jgit_docs_version[0], jgit_docs_version[1]])
        assets_to_update.update({jgit_asset.name: jgit_asset})

    return assets_to_update, skipped_assets


def print_dependency_management_report(updated_assets, assets_to_update, skipped_assets, checking_only, ignore_errors=False):
    """
    ...
    print("Updated Assets:") # N.B: Release tooling expects this header even if empty.
    if updated_assets:
    ...
    if not_updated_assets:
        ...
        for name in not_updated_assets:
            ...
            split_artifact = asset.artifact.split(':')
            print("    {0}:{1} {2} -> {3}".format(split_artifact[0], split_artifact[1], asset.old_version, split_artifact[2]), flush=True)

    print("Skipped Assets: (Already same version as the bom)") # N.B: Release tooling expects this header.
    if skipped_assets:
        for name, asset in list(skipped_assets.items()):
            print("    {0}".format(asset.artifact), flush=True)

    if checking_only and len(assets_to_update) > 0:
        # Just print the report for advice, but don't fail the build
        print("Found {0} dependencies not matching the version in BOM. Please review if these are CVE fixes.".format(len(assets_to_update)), file=stderr)
        exit(0 if ignore_errors else 1)
...
def eval_maven_project_version(pom):
    """
    Gets the version of a project by evaluating the `pom` file given.
    """
    output = None
    cmd = ["./mvnw", "-f", pom, "help:evaluate", "-q", "-DforceStdout", "-Dexpression=project.version"]
    try:
...
def check_dependencies_against_bom(asset_filter, build_file_assets):
    ...
    # Check JGit version
    assets_to_update, skipped_assets = check_jgit_docs_version(assets_to_update, skipped_assets)

    # Print a report of any assets to update/need updating or skipped.
    print_dependency_management_report({}, assets_to_update, skipped_assets, args.check, args.ignore_errors)
...
def update_dependencies_from_bom(asset_filter, build_file_assets):
    ...
    updated_assets = update_bazel_file("tools/nongoogle.bzl", assets_to_update)

    # Check JGit version
    assets_to_update, skipped_assets = check_jgit_docs_version(assets_to_update, skipped_assets)

    # Print a report of any assets to update/need updating or skipped.
    print_dependency_management_report(updated_assets, assets_to_update, skipped_assets, args.check, args.ignore_errors)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ignore_errors', default=False, action='store_true',
                        help="Exit successfully even if a check fails, e.g. dependency mismatch against BOM.")
