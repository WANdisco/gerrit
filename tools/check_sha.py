...
def check_jgit_docs_version(assets_to_update, skipped_assets):
    """
    Checks the JGit docs version against the JGit version from 'make display_version'
    ...
    submodule_version = get_jgit_submodule_version()
    jgit_submodule_version = parse_jgit_version(submodule_version)
    if not jgit_submodule_version:
        print("Couldn't parse the JGit version from the 'make display_version' output. version=" + submodule_version, file=stderr)
        return assets_to_update, skipped_assets
...
def print_dependency_management_report(updated_assets, assets_to_update, skipped_assets, checking_only):
    ...
    if updated_assets:
        for name, asset in updated_assets.items():
