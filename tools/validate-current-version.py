  ...
  if os.path.exists(script_path):
    try:
      ...
      if not git_describe_version == current_bzl_version:
        # If the only difference is that 'git describe' still has the SNAPSHOT suffix then we're in
        # TC/RC state but can't do the final release tag until later.
        if current_bzl_version == git_describe_version.rstrip("-SNAPSHOT"):
            # Versions are equal but we haven't yet tagged the final release build.
            print("WARNING: Gerrit bzl ({0}) and tag ({1}) versions match, but tag is still in snapshot during TC/RC.".format(current_bzl_version, git_describe_version))
        else:
