...
# Common flags to control bazel build/test execution:
GERRIT_BAZEL_FLAGS :=
GERRIT_BAZEL_TEST_FLAGS := --cache_test_results=NO --test_env=GERRITMS_REPLICATION_DISABLED=true --test_tag_filters=-elastic,-docker,-disabled,-replication

# Restrict resource usage if running in a Jenkins container:
ifneq ($(JENKINS_URL),)
  GERRIT_BAZEL_FLAGS += --local_cpu_resources=4
endif
...
# Either local build or officialBuild can optionally skip dependency management, echo out why skipped:
...
# If we're doing a release build, then we want to warn about any outstanding updates as they may be
# critical CVEs, however we should have ran this tool much earlier with dependencyManagement on to
# keep bazel deps up to date with what's in the BOM.
	./tools/check_sha.py --check --ignore_errors --update_version="$(alm_common_bom_version)" --filter=$(dependency_management_filter)
else
# If we do a local build with dependency management on, go ahead and update versions and we can
# push up a review to assess the updated libraries.
...
fast-assembly-gerrit:
	...
	bazelisk build $(GERRIT_BAZEL_FLAGS) release
	@echo
...
# if you are just debugging locally but best to get the full scan of API dependencies build and put into the api.zip.
	bazelisk build $(GERRIT_BAZEL_FLAGS) api
...
nuclear-clean: clean
	...
	@git clean -fdx polygerrit-ui polymer-bridges
...
setup-env:
	...
	$(eval alm_common_bom_version=$(shell ./mvnw help:evaluate -Dexpression=alm-common-bom-version -DforceStdout -q))
...
else ifeq ($(RUN_INTEGRATION_TESTS),verify)
decide-run-tests: run-verification-tests
else
...
run-verification-tests:
	@echo
	@echo "************ Verification Tests Starting **************"
	...
	bazelisk test $(GERRIT_BAZEL_FLAGS) $(GERRIT_BAZEL_TEST_FLAGS) \
...
detect-flaky-acceptance-tests:
	...
	bazelisk test $(GERRIT_BAZEL_FLAGS) $(GERRIT_BAZEL_TEST_FLAGS) \
	        --runs_per_test=10 --runs_per_test_detects_flakes=true \
	        //...
	@echo
