...
function prereqs() {
  ...
  info " * Have a version of GitMS (1.11.0.2 or higher) installed and running"
...
function check_java() {
  ...
  #Read the javaHome property from gerrit.config and set JAVA_HOME accordingly
  ...
  if [[ -n "$JAVA_HOME" ]]; then
    ...
    if [[ "${#known_java_version_split[@]}" -gt 1 ]];then
      major="${known_java_version_split[0]}"
      if [[ "$major" -ne 11 ]]; then
          perr "We require that you use Java 11 with GerritMS. JAVA_HOME is currently ${known_java_version}."
          exit 1
      fi
...
function run_gerrit_init() {
  ...
  local ret_code
  ${JAVA_BIN} -jar "${GERRIT_ROOT}/bin/gerrit.war" init -d "${GERRIT_ROOT}" --batch --reindex-threads=-1
  ret_code="$?"
...
function get_gerrit_version() {
  ...
  local gerritversion=$("${JAVA_HOME}/bin/java" -jar "$tmpwar" version 2>&1)
...
function check_db_config() {
  echo " Checking Gerrit configuration..."
 ...
 # If any of these configurations are set in the gerrit.config then check that they have the desired value.
  if [[ -n "$notedb_changes_write"  && "$notedb_changes_write" != true ||
        ...
        -n "$notedb_primaryStorage" && "$notedb_primaryStorage" != "NOTE_DB" ||
...
GERRIT_RELEASE_NOTES="https://www.gerritcodereview.com/3.7.html"
GERRITMS_INSTALL_DOC="http://docs.wandisco.com/gerrit/1.12/#doc_gerritinstall"
...
PREVIOUS_ALLOWED_RP_GERRIT_VERSIONS=( )
