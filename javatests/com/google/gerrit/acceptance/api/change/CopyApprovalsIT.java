...
public class CopyApprovalsIT extends AbstractDaemonTest {
  ...
  public void multipleProjects() throws Exception {
    ...
    assertThat(gApi.changes().id(change1.getChangeId()).current().votes()).isNotEmpty();;
    assertThat(gApi.changes().id(change2.getChangeId()).current().votes()).isNotEmpty();

    // these amends are reworks so votes will not be copied on first pass.
    amendChange(change1.getChangeId());

    // these wont be copied on first pass.
    ...
    try (ProjectConfigUpdate u = updateProject(allProjects)) {
      u.getConfig().updateLabelType(LabelId.CODE_REVIEW, b -> b.setCopyCondition("changekind:NO_CODE_CHANGE OR is:ANY"));
      u.save();
    }

    // lets run copy approvals on the projects required.
    recursiveApprovalCopier.persist(project, null);
    RecursiveApprovalCopier.RecursiveApprovalResults project1Results =
        recursiveApprovalCopier.getRecursiveApprovalResults();

    // run again on the second project.
    recursiveApprovalCopier.persist(secondProject, null);
    RecursiveApprovalCopier.RecursiveApprovalResults project2Results =
        recursiveApprovalCopier.getRecursiveApprovalResults();

    // Ensure that the sliced ref updates have been created and completed for these
    // projects.
    assertThat(project1Results.totalRefUpdates).isEqualTo(1);
    assertThat(project1Results.finishedRefUpdates).isEqualTo(1);
    // This should have refupdates from the first run + the second run.
    assertThat(project2Results.totalRefUpdates).isEqualTo(2);
    assertThat(project2Results.finishedRefUpdates).isEqualTo(3);
  ...
  public void refUpdateNotified() throws Exception {
    ...
    assertThat(gApi.changes().id(change.getChangeId()).current().votes().size()).isGreaterThan(0);
