...
import static com.google.gerrit.server.notedb.IntBlob.SEQUENCE_TUPLE_DELIMITER;
...
public class RevertRepoSequenceTest extends AbstractReplicationSetup{
    ...
    public void setUp() throws Exception {
        ...
        AbstractReplicationSetup.setupReplicatedEventsCoordinatorProps(false, extraProperties, false);
    }

    @Test
    public void sequenceShouldBeFirstChangeId() throws Exception {
      RepoSequence initSequence = newSequence(RepoSequence.NAME_CHANGES, INITIAL_SEQUENCE, 1);
      assertThat(initSequence.next()).isEqualTo(INITIAL_SEQUENCE);
    }

    @Test
    public void firstSequenceBlobShouldBeTuple() throws Exception {
        newSequence(RepoSequence.NAME_CHANGES, INITIAL_SEQUENCE, 1).next();
        assertThat(readBlob(RepoSequence.NAME_CHANGES)).isEqualTo(dummyTestCoordinator.getThisNodeIdentity() + SEQUENCE_TUPLE_DELIMITER + (INITIAL_SEQUENCE+1));
    }

    @Test
    public void shouldRevertTupleToIntAndBeRolledOn() throws Exception {
        RepoSequence sequence = newSequence(RepoSequence.NAME_CHANGES, INITIAL_SEQUENCE, 1);
        sequence.next();
        assertThat(readBlob(RepoSequence.NAME_CHANGES)).isEqualTo(dummyTestCoordinator.getThisNodeIdentity() + SEQUENCE_TUPLE_DELIMITER + (INITIAL_SEQUENCE+1));

        revertRepoSequence.updateSequence(allProjectsName, RepoSequence.NAME_CHANGES);

        // updating the sequence will also roll the sequence number forward
        assertThat(Integer.parseInt(readBlob(RepoSequence.NAME_CHANGES))).isEqualTo(INITIAL_SEQUENCE + 2);
    }
