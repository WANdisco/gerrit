...
import com.google.gerrit.server.replication.ProjectNameCallback;
...
public class ReplicatedEventsCoordinatorImpl implements ReplicatedEventsCoordinator {
  ...
  private final ReplicatedCacheWatcher replicatedCacheWatcher;
  ...
  @SuppressWarnings("StaticAssignmentInConstructor") // TODO Does gerritVanillaServerConfig need to be static if it's in a singleton...
  @Inject
  public ReplicatedEventsCoordinatorImpl(@GerritServerConfig Config config,
    ...
    if(!isReplicationEnabled()){
      ...
      replicatedCacheWatcher = null;
      return;
    }

    ensureEventsDirectoriesExist();

    // Instance of cache watcher needs to be constructed before the CacheEvent Processor.
  ...
  public void start() {
    ...
    replicatedConfiguration.logReplicatedConfigurationSettings();
  ...
  public void stop() {
    ...
    // Replicated scheduler has specific shutdown ordering within it, to cancel off the workers,
    replicatedScheduling.stop();

    // lets kill our processors - this must be after the scheduler shutdown, as inprogress work uses these processors,
    // and we often end up with NPEs, if we unsubscribe and empty out the processor map while events are trying to reference
  ...
  @Override
  public <K, V> Cache<K, V> createReplicatedCache(String cacheName, Cache<K, V> cache, String projectToReplicateAgainst, Class<K> keyClass) {
    return isReplicationEnabled() ? new ReplicatedCacheImpl<>(this, cacheName, cache, projectToReplicateAgainst, keyClass) : cache;
  }

  @Override
  public <K, V> Cache<K, V> createReplicatedCache(String cacheName, Cache<K, V> cache, ProjectNameCallback<K> projectToReplicateAgainst, Class<K> keyClass) {
    return isReplicationEnabled() ? new ReplicatedCacheImpl<>(this, cacheName, cache, projectToReplicateAgainst, keyClass) : cache;
  }

  @Override
  public <K, V> LoadingCache<K, V> createReplicatedLoadingCache(String cacheName, LoadingCache<K, V> cache, String projectToReplicateAgainst, Class<K> keyClass) {
    return isReplicationEnabled() ? new ReplicatedLoadingCacheImpl<K, V>(this, cacheName, cache, projectToReplicateAgainst, keyClass) : cache;
  }

  @Override
  public <K, V> LoadingCache<K, V> createReplicatedLoadingCache(String cacheName, LoadingCache<K, V> cache, ProjectNameCallback<K> projectToReplicateAgainst, Class<K> keyClass) {
    return isReplicationEnabled() ? new ReplicatedLoadingCacheImpl<K, V>(this, cacheName, cache, projectToReplicateAgainst, keyClass) : cache;
  }
