...
public class ReplicatedIncomingCacheEventProcessor extends AbstractReplicatedEventProcessor {
  ...
  public void processIncomingReplicatedEvent(final ReplicatedEvent replicatedEvent) {
    applyCacheMethodOrCacheOperation((CacheKeyWrapper) replicatedEvent);
  }

  private void applyCacheMethodOrCacheOperation(CacheKeyWrapper cacheKeyWrapper) {
    ...
    if(cacheKeyWrapper.value != null){
      // If the cacheKeyWrapper has a value then we can perform a "put" operation. If value is null then we will
      // want to perform an invalidation to evict the entry from the cache. In the case of web_sessions cache, a login
      // performs a cache put, whereas a logout will result in a cache invalidate operation.
      applyReplicatedCachePut(cacheKeyWrapper.cacheName,
              cacheKeyWrapper.getKeyAsOriginalType(), cacheKeyWrapper.getValueAsOriginalType());
    } else {
  ...
  private <K, V> void applyReplicatedCachePut(String cacheName, K key, V value){
    ReplicatedCacheWrapper<K, V> wrapper = replicatedCacheWatcher.getReplicatedCache(cacheName);

    // IMPORTANT NOTE: If the cache is a web_session cache, then it will not exist as a cache or indeed
    // a replicated cache until a browser is opened for the remote site The WebSessionManager is not created
    // until a browser session is open. To know if the cache has been created, look for the following
    // logging to ensure that the cache is being watched on a given site:
    // * e.g. ReplicatedCacheWatcher : CACHE New cache named web_sessions inserted
    ...
    if (wrapper == null) {
      ...
      throw new ReplicatedEventsUnknownTypeException(
              String.format("CACHE call for replicated PUT could not be made, as cache does not exist. %s", cacheName));
    }

    logger.atFine().log("CACHE %s : performing cache PUT with key %s and value %s", cacheName, key, value);
    wrapper.put(key, value);
  }

  private <K, V> void applyReplicatedEvictionFromCache(String cacheName, K key) {
    ...
    if (replicatedEventsCoordinator.isCacheToBeEvicted(cacheName)) {
      ...
      if (key instanceof List<?>) {
        // Ensuring type safety by using keyClass cast
        List<K> keyList = new ArrayList<>();
        for (Object obj : (List<?>) key) {
          if (wrapper.getKeyClass().isInstance(obj)) {
            keyList.add(wrapper.getKeyClass().cast(obj));
          } else {
            throw new ClassCastException("Invalid key type: " + obj.getClass().getName() +
                    ", expected: " + wrapper.getKeyClass().getName());
          }
        }

        logger.atFine().log("CACHE %s, invalidateAll keys %s...", cacheName, keyList);
        wrapper.invalidateAll(keyList);

        if (replicatedEventsCoordinator.getReplicatedConfiguration().isCacheToBeReloaded(cacheName)) {
          logger.atFine().log("CACHE %s get keys %s...", cacheName, keyList);
          wrapper.getAll(keyList);
        } else {
          logger.atFine().log("CACHE %s *not* to get keys %s...", cacheName, keyList);
        }

      } else {
        //Single key case: key is not an instanceof List so can call the regular invalidate.
        if (!wrapper.getKeyClass().isInstance(key)) {
          throw new ClassCastException("Invalid key type: " + key.getClass().getName() +
                  ", expected: " + wrapper.getKeyClass().getName());
        }
